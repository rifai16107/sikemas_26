<?php
    ...
    if(empty($_SESSION['admin'])){
                                            ...
                                            if($file != ""){
                                                ...
                                                if(in_array($eks, $ekstensi) == true){
                                                    if($ukuran < 5000000){
                                                        ...
                                                        $id_nodin = $_REQUEST['id_nodin'];
                                                        $query = mysqli_query($config, "SELECT file FROM tbl_surat_nodin WHERE id_nodin='$id_nodin'");
        ...
        $id_nodin = mysqli_real_escape_string($config, $_REQUEST['id_nodin']);
        $query = mysqli_query($config, "SELECT id_nodin, no_agenda, no_surtug,jenis_surat,tgl_surtug,kode,kode_spd,no_lamp_spd,nosu_lengkap,tujuan_tgs,tgl_mulai,tgl_selesai, file, beban_ang,id_user FROM tbl_surat_nodin WHERE id_nodin='$id_nodin'");
        list($id_nodin, $no_agenda, $no_surtug, $jenis_surat, $tgl_surtug, $kode, $kode_spd, $no_lamp_spd, $nosu_lengkap, $tujuan_tgs, $tgl_mulai, $tgl_selesai, $file, $beban_ang, $id_user) = mysqli_fetch_array($query);

        if($_SESSION['id_user'] != $id_user AND $_SESSION['id_user'] > 2){
            echo '<script language="javascript">
                    window.alert("ERROR! Anda tidak memiliki hak akses untuk mengedit data ini");
                    window.location.href="./admin.php?page=tsn";
                  </script>';
        } else {?>
            ...
            <div class="row jarak-form">
                ...
                <!-- Form START -->
                <form class="col s12" method="POST" action="?page=tsn&act=edit" enctype="multipart/form-data">

                    <!-- Row in form START -->
                    <div class="row">
                        <div class="input-field col s6">
                            <input type="hidden" name="id_nodin" value="<?php echo $id_nodin ;?>">
                        ...
                        <label for="kode_spd">Kode MAK Kegiatan</label>
                        </div>
